...
public class EmbeddedResourceValidator extends ResourceValidatorImpl {
  ...
  protected void validate(final Resource resource, final CheckMode mode, final CancelIndicator monitor, final IAcceptor<Issue> acceptor) {
    ...
    final EObject embeddedModel = IterableExtensions.<EObject>head(_contents);
    ...
    if ((matchingObject == null)) {
      ...
      int _length = visibleRegion.getLength();
      this.error("The selected element cannot be removed or renamed in this view.", _offset, _length, acceptor);
    } else {
      ...
      if ((selectedNode != null)) {
        ...
        boolean _greaterThan = (selectedOffset > _offset_1);
        if (_greaterThan) {
          int _offset_2 = visibleRegion.getOffset();
          final String prefix = embeddedText.substring(_offset_2, selectedOffset);
          final int start = this.nonWhitespaceStart(prefix);
          if ((start >= 0)) {
            int _offset_3 = visibleRegion.getOffset();
            int _plus = (_offset_3 + start);
        ...
        int _length_1 = selectedNode.getLength();
        final int selectedEndOffset = (selectedOffset + _length_1);
        int _offset_4 = visibleRegion.getOffset();
        int _length_2 = visibleRegion.getLength();
        final int visibleEndOffset = (_offset_4 + _length_2);
